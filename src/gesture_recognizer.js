// https://raw.githubusercontent.com/pfgithub/threadclient/main/packages/animator-ui-web/src/gesture_recognizer.tsx
...
// TODO: onStartGesture: () => onUpdate(), onFinish()
function recognizeGestures(onEvent) {
    const cleanup = [];

    const pointers = new Map();
    const getPointerMap = (type) => {
    ...
    const eventFromMap = (map, opts) => {
        ...
        if(relevant.length === 2) {
            return {
                kind: "touchzoom",
                start: first_touch.map(itm => itm.point),
                end: relevant.map(itm => itm[itm.length - 1].point),
                ...opts,
            };
        }
